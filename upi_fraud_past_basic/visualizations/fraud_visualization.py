...
# Define paths
RESULTS_FILE = "C:/upi_fraud_past_basic/results/predictions_20250318_2302.csv"
PERFORMANCE_FILE = "C:/upi_fraud_past_basic/model/model_performance.csv"
SAVE_FOLDER = "C:/upi_fraud_past_basic/visualizations/"
INDIA_MAP_PATH = "C:/upi_fraud_past_basic/data/map_of_india.jpg"
...
ax.legend(handles=legend_patches, loc="lower left", fontsize=8, title="City Legend", frameon=True)
...
if "Transaction_Type" in df.columns:
    ...
    transaction_limits = {"UPI": 70, "Cash": 80, "Online": 60}
    ...
    print("⚠️ WARNING: 'Transaction_Type' column missing, skipping fraud type comparison.")
...
performance_df.set_index("Model")[["Accuracy", "Precision", "Recall", "F1_Score"]].plot(kind="bar", figsize=(10, 6))
...
# ✅ Fraud Rate by Device & Transaction Type
if "Device_Type" in df.columns and "Transaction_Type" in df.columns:
    pivot_table = df.pivot_table(index="Device_Type", columns="Transaction_Type", values="Fraud_Prediction_RF", aggfunc="mean")
    plt.figure(figsize=(10, 6))
    sns.heatmap(pivot_table, annot=True, cmap="coolwarm", fmt=".2f")
    plt.title("Fraud Rate by Device & Transaction Type")
    plt.xlabel("Transaction Type")
    plt.ylabel("Device Type")
    plt.savefig(os.path.join(SAVE_FOLDER, "fraud_heatmap.png"))
    plt.show()
else:
    print("⚠️ WARNING: Missing 'Device_Type' or 'Transaction_Type' column, skipping heatmap.")
